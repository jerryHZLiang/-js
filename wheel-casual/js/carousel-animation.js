  ...
  slide(step = 10) {
    return function(fromNode, toNode, onFinish) {
      ...
      function fromNodeAnimation() {
        if (offset1 < offsetX) {
          fromNode.style.left =
            parseInt(getComputedStyle(fromNode).left) - step + "px";
          offset1 -= step;
          requestAnimationFrame(fromNodeAnimation);
        }
      }
...
const c = new Carousel(document.querySelector(".carousel"), Animation.fade);
 const c2 = new Carousel(document.querySelectorAll('.carousel')[1], Animation.slide(20))
